function [v,l,hist] = SCF_lagrange(A, v0, options)
    % SCF_lagrange   Method to find smallest eigenvalue and corresponding
    % eigenvector of NEPv A(v). Based on standard SCF, but a matrix
    % Lagrange polynomial is build using a number of previous points.
    %
    % [v,l,hist] = SCF_lagrange(A,v0 [,options])
    %
    % options(struct) are
